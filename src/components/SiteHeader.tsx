...
import { ShoppingBag, LayoutDashboard, UserRound } from "lucide-react";
    ...
    <header className="sticky top-0 z-40 border-b border-border/60 bg-background/85 backdrop-blur">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4">
        ...
        <nav className="hidden items-center gap-7 text-sm md:flex">
          ...
          {user && <NavItem to="/minha-conta"><UserRound className="h-4 w-4" /> Minha conta</NavItem>}
          {isAdmin && <NavItem to="/admin"><LayoutDashboard className="h-4 w-4" /> Admin</NavItem>}
        </nav>

        <div className="flex items-center gap-2">
          {!user && <Link to="/entrar" className="hidden text-sm font-medium sm:inline">Entrar</Link>}
          <Link
            to="/carrinho"
